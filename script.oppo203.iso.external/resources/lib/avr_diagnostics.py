...
def wizard_family_options() -> tuple[dict[str, object], ...]:
    ...
    for preset_id in list_avr_presets():
        ...
        options.append({
            ...
            "experimental": bool(preset.get("experimental") or preset.get("requires_experimental_acknowledgement")),
            "requires_sony_experimental_acknowledgement": preset.get("backend") == "sony_audio_api",
            "hardware_validation_claimed": False,
        })
    return tuple(options)


def apply_wizard_selection(settings: dict[str, object] | object | None, selection: dict[str, object]) -> dict[str, object]:
    """Apply AVR wizard answers to a settings dict without executing commands."""
    current = _settings_dict(settings)
    backend = str(selection.get("avr_backend") or selection.get("backend") or selection.get("id") or AVR_BACKEND_DISABLED)
    ...
    for key in ("avr_host", "avr_port", "avr_player_input", "avr_restore_input", "avr_sound_mode", "sony_avr_psk", "sony_avr_api_path", "sony_avr_player_input_uri", "sony_avr_restore_input_uri"):
        if key in selection:
            current[key] = str(selection.get(key, ""))
    current["avr_restore_enabled"] = "true" if _truthy(selection.get("avr_restore_enabled", False)) else "false"
    current["avr_power_off_enabled"] = "true" if _truthy(selection.get("avr_power_off_enabled", False)) else "false"
    current["avr_volume_automation_enabled"] = "true" if _truthy(selection.get("avr_volume_automation_enabled", False)) else "false"
    current["sony_avr_experimental_acknowledged"] = "true" if _truthy(selection.get("sony_avr_experimental_acknowledged", current.get("sony_avr_experimental_acknowledged", "false"))) else "false"
    return current
...
def settings_summary(settings: dict[str, object] | object | None) -> dict[str, object]:
    data = _settings_dict(settings)
    keys = (
        "avr_control_enabled", "avr_backend", "avr_host", "avr_port", "avr_player_input",
        "avr_restore_enabled", "avr_restore_input", "avr_power_on_enabled", "avr_power_off_enabled",
    ...
    for key in keys:
        ...
        elif key in {"avr_host", "avr_player_input", "avr_restore_input", "sony_avr_player_input_uri", "sony_avr_restore_input_uri"}:
...
def dry_run_avr_action(settings: dict[str, object] | object | None, action: str) -> dict[str, object]:
    data = _settings_dict(settings)
    ...
    return sanitize_payload({
        ...
        "requires_explicit_user_action": action in {"power_on", "select_input", "restore_input", "power_off"},
        "hardware_validation_claimed": False,
...
def run_query_only_test(settings: dict[str, object] | object | None, query: str = "power", *, controller: object | None = None) -> dict[str, object]:
    ...
    if ctrl is None:
        return sanitize_payload(_blocked("query_" + query, data, "avr_controller_unavailable"), data)
...
def run_explicit_test_action(settings: dict[str, object] | object | None, action: str, *, explicit_user_action: bool = False, controller: object | None = None) -> dict[str, object]:
    """Run a state-changing AVR test only after explicit user action."""
    data = _settings_dict(settings)
    if action in {"power_on", "select_input", "restore_input", "power_off"} and not explicit_user_action:
...
def format_report(report: dict[str, object], settings: dict[str, object] | object | None = None) -> str:
...
def save_report(report: dict[str, object], root_dir: str, *, now: float | Callable[[], float] | None = None, writer: Writer | None = None, settings: dict[str, object] | object | None = None) -> str:
...
def export_avr_diagnostic_report(settings: dict[str, object] | object | None, root_dir: str, *, now: float | Callable[[], float] | None = None, writer: Writer | None = None) -> str:
    return save_report(build_diagnostic_report(settings), root_dir, now=now, writer=writer, settings=settings)
