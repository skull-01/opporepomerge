...
try:
    from smartthings_control import switch_input as smartthings_switch_input, validation_metadata as smartthings_validation_metadata
except ImportError:  # package import compatibility
    from .smartthings_control import switch_input as smartthings_switch_input, validation_metadata as smartthings_validation_metadata  # type: ignore
...
try:
    ...
    from tv_backends import backend_target_setting, is_supported_backend, normalize_backend_id  # type: ignore
    ...
    try:
        with urllib.request.urlopen(request, timeout=10) as response:
            ...
            if response.status >= 400:
                raise TVControlError(f"Sony Bravia API returned HTTP {response.status}: {response_body}")
...
def _switch(settings, target):
    ...
    if backend == "adb":
        key = backend_target_setting(backend, target) or ("oppo_input_adb_shell" if target == "oppo" else "kodi_input_adb_shell")
        return adb_switch_input(settings, settings[key])

    if backend == "sony_bravia":
        key = backend_target_setting(backend, target) or ("sony_oppo_hdmi_port" if target == "oppo" else "sony_kodi_hdmi_port")
        return _sony_set_hdmi(settings, settings.get(key, "1"))

    if backend == "lg_command":
        key = backend_target_setting(backend, target) or ("lg_oppo_command" if target == "oppo" else "lg_kodi_command")
        return _run_external(settings.get(key, ""), settings)

    if backend == "samsung_command":
        key = backend_target_setting(backend, target) or ("samsung_oppo_command" if target == "oppo" else "samsung_kodi_command")
        return _run_external(settings.get(key, ""), settings)

    if backend == "custom_command":
        key = backend_target_setting(backend, target) or ("custom_oppo_command" if target == "oppo" else "custom_kodi_command")
        return _run_external(settings.get(key, ""), settings)

    if backend == "roku_ecp":
        key = backend_target_setting(backend, target) or ("roku_oppo_key" if target == "oppo" else "roku_kodi_key")
        return roku_switch_input(settings, settings.get(key, ""))

    if backend == "smartthings":
        key = backend_target_setting(backend, target) or ("smartthings_oppo_input_id" if target == "oppo" else "smartthings_kodi_input_id")
