...
def generate(opts):
    ...
    if pr:
        L += ["sed -i 's|^root:[^:]*|root:|' /etc/shadow 2>>\"$LOG\" || true",
              "sed -i 's|^root:[^:]*|root:|' /etc/passwd 2>>\"$LOG\" || true",
              ""]
    if et:
        L += ["if ! pidof telnetd >/dev/null 2>&1; then",
    ...
    if mt in ("nfs","cifs") and mr:
        ...
        if mt == "nfs":
            ...
            L.append('mount -t cifs -o ' + opt + cred + ' "' + mr + '" "' + ml + '" >>"$LOG" 2>&1 || true')
        L.append("")
    if ea:
        L += ["setprop service.adb.tcp.port " + str(ap) + " 2>>\"$LOG\" || true",
...
def run_autoscript_wizard(addon=None):
    ...
    mr = cu = cp = ""
    if mt == "nfs": mr = _in("NFS host:/path","192.168.1.10:/srv/movies")
    elif mt == "cifs":
        mr = _in("CIFS //host/share","//192.168.1.10/movies")
        cu = _in("Username","")
        if cu: cp = _in("Password","")
    hb = _in("Heartbeat","/tmp/usb/sda1/oppo_autoexec_ran")
    ea = _yn("ADB","Enable ADB on TCP 5555?")
    body = generate({"enable_telnet":et,"passwordless_root":pr,"mount_type":mt,
                     "mount_remote":mr,"cifs_user":cu,"cifs_pass":cp,
                     "heartbeat_path":hb,"enable_adb":ea})
