...
class WizardState:
    ...
    def copy(self):
        c = WizardState(values=self.values, dry_run=self.dry_run, step_index=self.step_index)
        c.history = list(self.history)
        c.last_test = None if self.last_test is None else dict(self.last_test)
        return c
...
def apply(state, *, writer):
    ...
    for k, v in state.values.items():
        writer(k, v)
        n += 1
    return {"written": True, "values": deepcopy(state.values), "dry_run": False, "writer_calls": n}
