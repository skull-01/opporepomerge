# CURRENT STATUS — v2.9.10 Build 9B SmartThings Experimental Request Helper and Fake API Tests

Current installable package: `script.oppo203.iso.external-2.9.10-build9b.zip`
Current artifact bundle: `script.oppo203.iso.external-2.9.10-build9b-artifacts-bundle.zip`
Current dev source: `script.oppo203.iso.external-2.9.10-build9b-dev-source.zip`
Current add-on version: `2.9.10`
Current build identity: `v2.9.10 Build 9B`
Baseline: `script.oppo203.iso.external-2.9.10-build9a-dev-source.zip`
Hardware validation status: not performed and not claimed.

## Build 9B summary

Build 9B completes the split SmartThings experimental backend slice by adding a guarded request helper and fake API tests. The helper requires explicit experimental acknowledgement before any request is attempted, builds a SmartThings device command POST request, redacts tokens from returned metadata, handles HTTP 401/403 safely, treats HTTP/network failures as non-fatal, and keeps hardware validation unclaimed.

No OPPO playback routing, service interception, playercorefactory.xml behavior, NAS/AutoScript behavior, OPPO command-map payloads, startup auto-power behavior, AVR sequencing, Android/ADB preset metadata, Roku ECP behavior, command/custom TV preset behavior, or runtime ZIP policy changed.
...
release-evidence/v2.9.10-build9b/MANIFEST.txt
...
Combined_AI_Handoff_and_Historical_Build_Reconstruction_v93_v2.9.10_BUILD9B.md
```
...
## Historical reconstruction entry — v2.9.10 Build 9B

```yaml
build_id: v2.9.10 Build 9B
package: script.oppo203.iso.external-2.9.10-build9b.zip
dev_source: script.oppo203.iso.external-2.9.10-build9b-dev-source.zip
artifact_bundle: script.oppo203.iso.external-2.9.10-build9b-artifacts-bundle.zip
baseline: script.oppo203.iso.external-2.9.10-build9a-dev-source.zip
scope: SmartThings experimental request helper and fake API tests
...
smartthings_request_helper: guarded_experimental
fake_api_tests: passed
token_leakage_in_results: blocked
```

## Resume prompt for next AI

```text
Continue the v2.9.10 Unified Hardware Ecosystem Expansion from v2.9.10 Build 9B.

Use this baseline:
script.oppo203.iso.external-2.9.10-build9b-dev-source.zip

Next build:
v2.9.10 Build 10 — TV diagnostics and dry-run validator.

Keep the build narrow. Add TV validation and dry-run reporting without launching ISO playback. Validate selected TV backend settings, provide a dry-run selected-backend report without network calls, add explicit test actions for switch-to-OPPO and switch-back-to-Kodi where already supported, and export a sanitized TV switching diagnostic report. Do not export SmartThings tokens, Sony PSKs, passwords, credentials, or sensitive command output. The report must state hardware_validation_claimed=false. Preserve existing TV backend registry APIs, SmartThings 9A/9B experimental guardrails, command/custom TV presets, Roku ECP backend behavior, Android / Google TV ADB preset metadata, OPPO playback routing, service interception, playercorefactory.xml behavior, NAS/AutoScript behavior, OPPO command-map payloads, startup auto-power behavior, AVR sequencing, runtime ZIP policy, and the 99% coverage gate.
