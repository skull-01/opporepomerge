...
- **Phase C (real Windows host) — the rebrand is end-to-end:** install the **v0.9.6** NSIS (or MSI) → confirm the **installer title**, **Start-Menu shortcut**, **uninstall entry** (Apps & features), **exe filename**, and the running app's **desktop window title + in-app title bar** all read **Kodi Oppo External Player Configurator** (the title bar shows `-v0.9.6`).
- **Upgrade across the rename:** install **v0.9.5** (OppoKodiAddon Configurator), then run **v0.9.6** → confirm the single old-version prompt still fires (NSIS detection is identifier-keyed, so it does) and settings survive. **Known gap:** a prior **MSI** install under the *old* DisplayName is not auto-detected by the preinstall hook's MSI branch (it matches `DisplayName == ${PRODUCTNAME}`, now the new name) — remove an old-named MSI manually if both are present.
- Verify the published MSI/NSIS SHA-256 against the release `SHA256SUMS.txt` / `configurator/release-evidence/v0.9.6/BUILD_NOTES.md`.

### Configurator — installer single old-version prompt (2026-06-03) → v0.9.5

Suppresses Tauri's built-in NSIS reinstall page so an upgrade shows **one** old-version prompt, not two (umbrella [ENH #334](https://github.com/skull-01/script.oppo203.iso.external/issues/334)). The reinstall page (`PageReinstall` / `PageReinstallUpdateSelection` / `PageLeaveReinstall` + the `Page custom` insertion — 202 lines) was removed from a **vendored** copy of the `@tauri-apps/cli` 2.11.2 NSIS template (`configurator/src-tauri/installer.nsi`, drift-guarded to the resolved CLI version), and `installer-hooks.nsh` `NSIS_HOOK_PREINSTALL` now removes a prior NSIS *or* MSI install behind one confirmation. Shipped as **configurator v0.9.5** (tag `configurator-v0.9.5`, bundles add-on **v2.9.16**). PRs: [#335](https://github.com/skull-01/script.oppo203.iso.external/pull/335) vendor + drift guard, [#336](https://github.com/skull-01/script.oppo203.iso.external/pull/336) suppress page + broaden hook, [#337](https://github.com/skull-01/script.oppo203.iso.external/pull/337) bump. Gate: `tsc -b` · **vitest 359** · **cargo 57** · `vite build`; **a local `tauri build --bundles nsis` (`makensis`) compiled the edited template + hook into a working installer** (the configurator PR gate does not run `tauri build` — only the release tag job and the local build do).

- **Phase C (real Windows host) — the single-prompt UX:** install **v0.9.4** (NSIS), then run the **v0.9.5** installer → confirm exactly **one** remove-previous-version prompt (no second Tauri reinstall page), that choosing **Yes** removes the old version and the upgrade completes, and that **app settings survive**. Also smoke a **fresh** install (no prior version → no prompt). Optional: with a prior **MSI** install present, confirm it is offered for removal too.
- **Known change:** the silent-install (`/S`) downgrade auto-abort (EarlyChecks) relied on the removed page's version compare and is now inert; GUI installs are unaffected.
- Verify the published MSI/NSIS SHA-256 against the release `SHA256SUMS.txt` / `configurator/release-evidence/v0.9.5/BUILD_NOTES.md`.
...
- **PR 1 — generator + contract** ([#310](https://github.com/skull-01/script.oppo203.iso.external/pull/310), #307). **No Phase C** (pure logic, cross-language-pinned).
- **PR 2 — panel + Desktop export** ([#311](https://github.com/skull-01/script.oppo203.iso.external/pull/311), #308). **Phase C:** (a) build an `autoexec.sh` in the **AutoScript** tab, **Export to Desktop**, confirm `<Desktop>/OppoKodiAddon-AutoScript/` has `autoexec.sh` (LF) + `HOW-TO-INSTALL.txt`; (b) follow the instructions — format a USB **FAT32**, copy `autoexec.sh` to its **root**, boot the player, confirm the heartbeat marker appears; (c) **Check availability** against a real OPPO — `#QVR` firmware capability + telnet/ADB/:436 ports. Confirm the CIFS password never appears outside the preview.
- **PR 3 — telnet** ([#312](https://github.com/skull-01/script.oppo203.iso.external/pull/312), #309). **Phase C:** with AutoScript's telnetd up on :2323, **Check telnet** confirms a live shell; **Install over telnet** (confirm) pushes `autoexec.sh` + chmod and the player picks it up.
- **Release — bump** ([#313](https://github.com/skull-01/script.oppo203.iso.external/pull/313)) **+ tag `configurator-v0.9.1`.** **Phase C:** install the published MSI/NSIS, open Developer Options → AutoScript; verify SHA-256 against the release's `SHA256SUMS.txt` / `release-evidence/v0.9.1/BUILD_NOTES.md`.

### Configurator — Developer Options console (2026-06-03, EOD #15) — Kodi/TV/OPPO/AVR/NAS consoles + Kodi dev tools + LAN scan → v0.9.0

Operator: "complete all the phases automatically then release it" (full auth). The LOCKED **Developer Options** initiative (umbrella [#290](https://github.com/skull-01/script.oppo203.iso.external/issues/290), `docs/BUILD_PLAN.md` §2) built as **7 CI-gated PRs** merged to `main`, then **configurator v0.9.0** built + published as Latest by CI on the `configurator-v0.9.0` tag (bundles add-on v2.9.15). Reach it from the header **Developer…** button. Software gates green throughout: `tsc -b` · **vitest 338** · **cargo test 50** · `vite build`; every panel browser-verified (nav, command-firing, the OPPO + NAS credential redaction, the Kodi scan). **Nearly all device behavior is Phase-C** — in-session only the UI + pure logic (command catalogs, subnet enumeration, JSON-RPC/command builders, transcript folding, redaction) were verifiable. ENH SHA-commented + left **OPEN**.

- **PR A — dev-tab shell** ([#298](https://github.com/skull-01/script.oppo203.iso.external/pull/298), issue #296). **No Phase C** (UI only — header entry, 5 tabs, routing).
- **PR B-OPPO — OPPO console** ([#299](https://github.com/skull-01/script.oppo203.iso.external/pull/299), #291). **Phase C (real OPPO):** fire TCP `#XXX` commands (power/transport/nav/queries/`#SVM`) + the raw box → confirm replies; fire HTTP catalog endpoints (`oppo_http_get`) → confirm bodies; start the TCP-push (`#SVM 3`) monitor and the HTTP poll → confirm live frames. Confirm `/loginSambaWithID` / `/loginNfsServer` queries render **redacted** in the transcript.
- **PR C — Kodi dev tools** ([#300](https://github.com/skull-01/script.oppo203.iso.external/pull/300), #292). **Phase C (real Kodi box over SSH):** Refresh shows installed-vs-bundled version; Read settings lists the box's settings (secrets masked); Register enables without restart; Restart Kodi (confirm) restarts; Upload a `.zip` (confirm) deploys + backs up the prior copy + re-registers.
- **PR D-TV — TV console** ([#301](https://github.com/skull-01/script.oppo203.iso.external/pull/301), #293). **Phase C (real TVs):** per backend (Roku ECP / ADB / Sony Bravia REST + IRCC / LG-Samsung-custom / SmartThings) fire the palette + raw commands → confirm the TV responds; SmartThings builds the request for display (the add-on fires it over HTTPS).
- **PR D-AVR — AVR console** ([#302](https://github.com/skull-01/script.oppo203.iso.external/pull/302), #294). **Phase C (real AVRs):** per backend (Denon/Marantz · Onkyo/Pioneer eISCP · Yamaha · Sony audio) select inputs from the palette + raw input box → confirm the receiver switches.
- **PR D-NAS — NAS panel** ([#303](https://github.com/skull-01/script.oppo203.iso.external/pull/303), #295). **Phase C (real LAN/NAS):** Scan network lists NAS hosts + detected protocols (SMB/NFS/AFP/FTP); Test login (SMB with creds / NFS reachability) lists the share. Confirm the password is **never** shown in the panel and never saved.
- **PR E — Kodi LAN scan** ([#304](https://github.com/skull-01/script.oppo203.iso.external/pull/304), #297). **Phase C (real LAN):** Scan network finds Kodi boxes on :8080 with their versions; clicking one fills the Kodi IP.
- **Release — bump** ([#305](https://github.com/skull-01/script.oppo203.iso.external/pull/305)) **+ tag `configurator-v0.9.0`.** **Phase C:** install the published MSI/NSIS on a real Windows host, smoke-test the wizard, open Developer Options; verify SHA-256 against the published release's CI-generated `SHA256SUMS.txt` / `release-evidence/v0.9.0/BUILD_NOTES.md`.
...
### Configurator infra + add-on hardening batch (2026-06-03, EOD #14) — CI, diagnostics, installer, i18n, tests → v0.8.6

Operator: "do all of this automatically." Seven themes, each a CI-gated PR; configurator **v0.8.6** built + published as Latest by the new release CI on the `configurator-v0.8.6` tag (bundles add-on v2.9.15).

- **CI / release automation — [PR #272](https://github.com/skull-01/script.oppo203.iso.external/pull/272)** (`.github/workflows/configurator-ci.yml`): windows-latest gate (npm ci → tsc -b + vite build → vitest → bundle add-on → cargo test) on every configurator PR; a `configurator-v*` tag builds MSI/NSIS + publishes the release. **No Phase C** — proven green on PRs #272–#278 and the v0.8.6 tag. The first run caught a real gap (tauri `build.rs` needs the bundled add-on zip) before it could bite a release.
- **Diagnostics export — [PR #273](https://github.com/skull-01/script.oppo203.iso.external/pull/273)** (`diagnostics.ts`, `DiagnosticsCard`, Rust `diagnostics_env`/`write_diagnostics`). Browser-verified the render + redaction. **Phase C (real Windows host):** on the dashboard click **Export diagnostics** → confirm the JSON writes to app-data, secret fields read `[redacted]`, and **Copy to clipboard** works.
- **Installer single prompt — [PR #274](https://github.com/skull-01/script.oppo203.iso.external/pull/274)** (`installer-hooks.nsh` scoped to the parallel-MSI case Tauri's reinstall page misses). makensis-verified. **Phase C (real upgrade):** install over a prior **NSIS** version → confirm **one** prompt; with an **MSI** install present → confirm the MSI-removal prompt also appears.
- **Add-on test hardening — [PR #276](https://github.com/skull-01/script.oppo203.iso.external/pull/276)** (`tests/test_property_http_hdmi.py`, property tests). Caught + fixed a real `OverflowError` on `int(float('inf'))` in `http_info_indicates_playing` (issue [#275](https://github.com/skull-01/script.oppo203.iso.external/issues/275)). **No Phase C** (pure tests + defensive fix; serial coverage 99%).
- **i18n scaffold — [PR #277](https://github.com/skull-01/script.oppo203.iso.external/pull/277)** (`i18n.ts` `t()` + English catalog; `WinShell` migrated). Browser-verified. **No Phase C** (English-only, text-identical).
- **Release v0.8.6 — [PR #278](https://github.com/skull-01/script.oppo203.iso.external/pull/278) bump + tag `configurator-v0.8.6`.** **Phase C:** install the published MSI/NSIS on a real Windows host and smoke-test the wizard; verify SHA-256 against `release-evidence/v0.8.6/BUILD_NOTES.md`.
