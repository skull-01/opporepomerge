...
**UX refinements ✅ DELIVERED — configurator v0.9.2 (umbrella #314; PRs #318 side-by-side `.dev-split` live-transcript layout · #319 Browse + add-on identity validation gate on the Kodi upload · #320 TV HDMI input switch via `planSwitch` + bump #321). New Rust `validate_addon_zip`/`pick_addon_zip` (dep `rfd`).**
**Build tag ✅ DELIVERED — configurator v0.9.3 (umbrella #322; PR #325 add-on packaging stamps `resources/oppokodiaddon.sig` (SHA-256 content manifest, `compute_manifest_sig`) · PR #326 configurator verifies it → signed/unsigned/mismatch, Rust `addon_manifest_sig` + dep `sha2` + bump #327). Cross-language guard pins the Python↔Rust manifest to one fixture hash. Not cryptographic (unsigned posture).**
...
optional tester OPPO TCP command list (else use the documented UDP-20x set), and filing the umbrella +
per-PR ENH issues.
