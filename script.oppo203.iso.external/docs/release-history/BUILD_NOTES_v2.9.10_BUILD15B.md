...
- Updated `resources/lib/avr_sony_audio.py` with guarded JSON POST helpers.
- Updated `resources/lib/avr_control.py` controller factory with explicit `sony_post` fake transport support.
- Updated `resources/lib/avr_presets.py` and `resources/lib/avr_types.py` to mark the Sony request helper available but experimental.
- Added `tests/test_v2910_build15b_sony_avr_request_helper.py`.
