...
class TOppoControlCoverageHardening(unittest.TestCase):
    ...
    def test_tcp_send_and_query_helpers_use_fake_socket(self):
        ...
        with (
            mock.patch.object(self.oc.socket, "create_connection", side_effect=fake_create),
    ...
    def test_response_status_preflight_and_verbose_helpers(self):
        ...
        with (
            mock.patch.object(self.oc, "query_power_status", return_value="ON"),
            mock.patch.object(self.oc, "query_input_source", return_value="BLU-RAY"),
        ):
            pre = self.oc.run_preflight(
                FakeSettings(dict(self.settings, oppo_preflight_enabled="true"))
            )
    ...
    def test_wol_discovery_and_http_helpers_are_local_fakes(self):
        ...
        with (
            mock.patch.object(self.oc.socket, "socket", side_effect=fake_socket),
        ...
        with mock.patch.object(
            self.oc.urllib.request, "urlopen", side_effect=self.oc.urllib.error.URLError("down")
        ):
            with self.assertRaises(self.oc.OppoError):
                self.oc._http_get(self.settings, "/fail")
        self.assertEqual(
            self.oc._translate_media_path(self.settings, "/media/Movie/BDMV/index.bdmv"),
            "/nas/Movie",
        )
    ...
    def test_http_info_track_command_and_filelist_helpers(self):
        ...
        self.assertEqual(
            list(self.oc._info_containers({"result": {"status": "PLAY"}}))[1]["status"], "PLAY"
        )
        self.assertEqual(
            self.oc._filter_commands_for_mode(
                self.settings, "oppo_start_commands", ["#PON", "#PLA"], {"already_on": True}
            ),
            ["#PLA"],
        )
        self.assertEqual(
            self.oc._resolve_hardware_wake_command(
                FakeSettings({"oppo_hardware_model": "chinoppo_m9702"}), "#PON"
            ),
            "#EJT",
        )
        with mock.patch.object(self.oc, "send_commands", return_value=["@OK"]) as send:
            out = self.oc.run_configured_commands(
                FakeSettings(dict(self.settings, oppo_start_commands="#PON\n#PLA")),
                "oppo_start_commands",
            )
        ...
        calls = []
        with (
            mock.patch.object(self.oc, "maybe_wake_on_lan", lambda s: calls.append("wol")),
            mock.patch.object(
                self.oc, "maybe_setup_verbose_mode", lambda s, h, p: calls.append("svm")
            ),
            mock.patch.object(
                self.oc, "run_configured_commands", lambda *a, **k: calls.append("tcp")
            ),
            ...
            mock.patch.object(
                self.oc, "play_media_http_api", lambda s, m: calls.append("play") or "ok"
            ),
        ):
        ...
        entries = self.oc.parse_undocumented_file_list(
            '{"files":[{"name":"Movie.iso","size":"10"}]}', base_path="/base"
        )
...
class TSettingsExternalTvRemoteCoverageHardening(unittest.TestCase):
    ...
    def test_tv_control_all_backends_with_no_real_tv(self):
        ...
        settings = FakeSettings(
            {
                ...
                "tv_ip": "192.0.2.30",
                "sony_psk": "psk",
                "oppo_input_adb_shell": "input-oppo",
                "kodi_input_adb_shell": "input-kodi",
                "sony_oppo_hdmi_port": "1",
                "sony_kodi_hdmi_port": "2",
                "lg_oppo_command": "echo lg-oppo",
                "lg_kodi_command": "echo lg-kodi",
                "samsung_oppo_command": "echo {tv_ip}",
                "samsung_kodi_command": "echo samsung-kodi",
                "custom_oppo_command": "echo custom-oppo",
                "custom_kodi_command": "echo custom-kodi",
            }
        )
        ...
        with mock.patch.object(
            tv_control.urllib.request, "urlopen", return_value=FakeHttpResponse("sony")
        ):
    ...
    def test_external_player_hold_modes_and_main_with_fakes(self):
            ...
            settings = FakeSettings(
                {
                    ...
                    "manual_stop_file": str(Path(td, "stop")),
                    "oppo_stop_commands": "#STP",
        ...
        with mock.patch.object(ep, "switch_to_oppo", side_effect=RuntimeError("adb down")):
            self.assertIsNone(
                ep._safe_tv_switch(
                    FakeSettings({"tv_backend": "adb", "tv_switching_enabled": "true"}), "oppo"
                )
            )
        with (
            mock.patch.object(ep, "run_start", lambda *a, **k: calls.append("start")),
        ...
        with (
            mock.patch.object(ep, "get_playback_info", return_value={"errcode1": -5}),
        ...
        with (
            mock.patch.object(ep, "query_playback_status", return_value="STOP"),
            mock.patch.object(ep.time, "sleep", lambda *_: None),
        ):
            ep.hold_playback(
                FakeSettings(
        ...
        stop = tempfile.NamedTemporaryFile(delete=False)
        stop.close()
        try:
            ep.hold_playback(
                FakeSettings({"hold_mode": "manual_file", "manual_stop_file": stop.name})
            )
        ...
        with mock.patch.object(ep.time, "sleep", lambda seconds: calls.append(seconds)):
            ep.hold_playback(
                FakeSettings({"hold_mode": "fixed_timeout", "fixed_timeout_minutes": "0"})
            )
        self.assertIn(1, calls)
        fake_settings = FakeSettings(
            {"addon_data_dir": tempfile.gettempdir(), "oppo_preflight_enabled": "true"}
        )
        with (
            mock.patch.object(
                ep.sys,
                "argv",
                ["external_player.py", "--addon-data", tempfile.gettempdir(), "--file", "/m.iso"],
            ),
            mock.patch.object(ep, "read_settings", return_value=fake_settings),
            mock.patch.object(
                ep,
                "run_preflight",
                return_value={"power_status": "ON", "input_source": "HDMI", "already_on": True},
            ),
    ...
    def test_oppo_remote_send_paths_with_local_settings(self):
        ...
        settings = FakeSettings(
            {
                "addon_data_dir": tempfile.gettempdir(),
                "remote_bridge_only_when_active": "true",
                "oppo_ip": "h",
                "oppo_port": "23",
                "oppo_socket_timeout": "1",
        ...
        with (
            ...
            mock.patch.object(
                remote, "_command_map", return_value={"power_on": "#PON", "missing": ""}
            ),
        ...
        with (
            mock.patch.object(
                ...
                "get_audio_tracks",
                return_value=[
                    {"index": 0, "name": "A", "selected": True},
                    {"index": 1, "name": "B", "selected": False},
                ],
            ),
        ...
        with (
            mock.patch.object(remote, "get_subtitle_tracks", return_value=[]),
            mock.patch.object(remote, "send_commands", lambda *a, **k: calls.append(a[2][0])),
        ):
            remote._cycle_subtitle(settings)
        with (
            mock.patch.object(remote, "set_play_time", side_effect=RuntimeError("nope")),
            mock.patch.object(remote, "_log", lambda msg: calls.append(msg)),
        ):
...
class TKodiBoundCoverageHardening(unittest.TestCase):
    def test_installer_builders_dialogs_and_writers_with_stubs(self):
        with kodi_stubs():
            ...
            xbmcaddon.reset(
                settings={"include_disc_folder_rules": "true", "python_path": "/usr/bin/python3"},
            ...
            installer.show_generated_xml()
            installer.show_generated_keymap_xml()
            ...
            self.assertTrue(
                any(call[0] in ("textviewer", "ok", "select") for call in xbmcgui.calls())
            )
    ...
    def test_installer_discovery_and_filelist_diagnostic_with_stubs(self):
        with kodi_stubs("oppo_control", "settings_reader"):
            ...
            fake_oc.discover_oppo = lambda timeout=0: [
                {"ip": "192.0.2.44", "port": 23, "name": "OPPO"}
            ]
    ...
    def test_installer_discovery_multiple_results_and_exceptions(self):
        ...
        with kodi_stubs("oppo_control"):
            ...
            installer.run_oppo_discovery()
            self.assertTrue(
                any(c[0] == "textviewer" and "multiple" in c[1].lower() for c in xbmcgui.calls())
            )

            # Single result with user declining apply -> textviewer branch.
            fake_oc.discover_oppo = lambda timeout=0: [
                {"ip": "192.0.2.40", "port": 9999, "name": "OPPO"}
            ]
            xbmcgui.reset()
            xbmcgui.push_yesno(False)
            installer.run_oppo_discovery()
    ...
    def test_installer_extra_branches_for_coverage(self):
        ...
        with kodi_stubs():
            ...
            self.assertEqual(
                installer.ADDON.getSetting("playback_architecture"), "service_interception"
            )
...
class TCoverageGateSecondPass(unittest.TestCase):
    def test_oppo_control_remaining_branches(self):
        ...
        self.assertEqual(
            oc.run_preflight(FakeSettings({"oppo_preflight_enabled": "false"})),
            {"power_status": None, "input_source": None, "already_on": False},
        )
        with (
            mock.patch.object(oc, "query_power_status", side_effect=OSError("down")),
            mock.patch.object(oc, "query_input_source", side_effect=oc.OppoError("bad")),
        ):
            pre = oc.run_preflight(
                FakeSettings(
                    {
                        "oppo_preflight_enabled": "true",
                        "oppo_ip": "h",
                        "oppo_port": "23",
                        "oppo_socket_timeout": "1",
                    }
        ...
        with mock.patch.object(oc, "wake_on_lan", lambda mac, b: "sent"):
            self.assertTrue(
                oc.maybe_wake_on_lan(
                    FakeSettings(
                        {
                            "oppo_use_wol": "true",
                            "oppo_mac": "001122334455",
        ...
        s = FakeSettings(
            {
                "oppo_http_disc_folder_root": "false",
                "oppo_http_path_from": "",
                "oppo_http_path_to": "",
                "oppo_http_urlencode_path": "false",
                "oppo_http_app_device_type": "3",
                "oppo_http_media_type": "4",
            }
        )
        ...
        with mock.patch.object(oc, "_http_get", return_value="not-json"):
            self.assertEqual(
                oc.get_playback_info(FakeSettings({"oppo_ip": "h"})), {"raw": "not-json"}
            )
        ...
        self.assertEqual(
            oc._filter_commands_for_mode(
                FakeSettings({"oppo_already_on_mode": "true"}),
                "oppo_start_commands",
                ["PON", "#PLA"],
            ),
            ["#PLA"],
    ...
    def test_oppo_control_filelist_edges(self):
        oc = importlib.import_module("resources.lib.oppo_control")
        entry = oc._normalise_filelist_entry(
            {"path": "/Movies/BDMV/", "isDir": True}, base_path="/base"
        )
        self.assertEqual(entry["entry_type"], "directory")
        self.assertEqual(
            oc._normalise_filelist_entry("LooseName", base_path="/base")["path"], "/base/LooseName"
        )
        ...
        self.assertEqual(
            oc._guess_name_from_fields(["size=10", "type=file", "/dir/movie.iso"]), "movie.iso"
        )
        self.assertEqual(oc._guess_name_from_fields(["", "index=1"]), "")
        self.assertEqual(
            oc._guess_path_from_fields(["x", "smb://server/share/movie.iso"]),
            "smb://server/share/movie.iso",
        )
    ...
    def test_external_player_more_hold_and_error_paths(self):
        ...
        with (
            mock.patch.object(ep.time, "time", side_effect=lambda: next(times)),
            mock.patch.object(ep.time, "sleep", lambda *_: None),
            mock.patch.object(
                ep, "get_playback_info", side_effect=[{"e_play_status": 0}, {"status": "STOP"}]
            ),
        ...
        with (
            mock.patch("oppo_tcp_client.OppoTcpClient", BadClient),
        ...
        calls = []
        with (
            mock.patch.object(ep, "run_configured_commands", lambda *a, **k: calls.append("stop")),
        ...
        with (
            ...
            mock.patch.object(
                ep,
                "read_settings",
                return_value=FakeSettings({"addon_data_dir": tempfile.gettempdir()}),
            ),
    ...
    def test_oppo_remote_kodi_and_success_specials(self):
        with kodi_stubs():
            ...
            xbmcaddon.reset(
                settings={"oppo_ip": "h", "remote_bridge_only_when_active": "false"},
                info={"profile": tempfile.mkdtemp()},
            )
            xbmcgui.reset()
            remote = importlib.import_module("resources.lib.oppo_remote")
            ...
            self.assertIn(
                "play", remote._command_map(FakeSettings({"oppo_remote_command_map": "not-json"}))
            )
            calls = []
            settings = FakeSettings(
                {
                    "addon_data_dir": "",
                    "remote_bridge_only_when_active": "false",
                    "oppo_ip": "h",
                    "oppo_port": "23",
                    "oppo_socket_timeout": "1",
                    "oppo_hardware_model": "udp_203",
            ...
            with (
                ...
                mock.patch.object(
                    ...
                    "_command_map",
                    return_value={
                        "cycle_audio": "__cycle_audio__",
                        "cycle_subtitle": "__cycle_subtitle__",
                        "seek_beginning": "__seek_beginning__",
                    },
                ),
                ...
                remote.send_remote_key("cycle_audio")
                remote.send_remote_key("cycle_subtitle")
                remote.send_remote_key("seek_beginning")
            ...
            with (
                mock.patch.object(
                    ...
                    "get_subtitle_tracks",
                    return_value=[
                        {"index": 0, "name": "Off", "selected": False},
                        {"index": 2, "name": "English", "selected": True},
                    ],
                ),
                mock.patch.object(
                    remote, "set_subtitle_track", lambda s, i: calls.append(("subtitle", i))
                ),
                mock.patch.object(remote, "_notify", lambda msg: calls.append(msg)),
            ):
                remote._cycle_subtitle(settings)
            with (
                mock.patch.object(remote, "set_play_time", lambda *a: calls.append("played")),
                mock.patch.object(remote, "_notify", lambda msg: calls.append(msg)),
            ):
                remote._seek_beginning(settings)
...
class TCoverageGateThirdPass(unittest.TestCase):
    def test_oppo_tcp_client_direct_state_machine_paths(self):
        ...
        with mock.patch.object(
            c2,
            "_connect_and_read",
            lambda: setattr(c2, "_error", OSError("down")) or c2._connected.set(),
        ):
    ...
    def test_oppo_tcp_client_connect_and_read_with_fake_socket(self):
        ...
        sleeps = []
        with (
            mock.patch.object(otc.time, "time", side_effect=[0, 0, 0.1, 0.2, 2]),
            mock.patch.object(
                otc.reconnect_backoff if hasattr(otc, "reconnect_backoff") else otc,
                "__name__",
                getattr(otc, "__name__", "oppo_tcp_client"),
                create=True,
            ),
        ):
            c4 = otc.OppoTcpClient("h", 23)
            self.assertFalse(
                c4.wait_for_stop_persistent(
                    timeout=1,
                    max_retries=2,
                    base_delay=5,
                    cap_delay=5,
    ...
    def test_settings_reader_and_i18n_remaining_edges(self):
        ...
        self.assertEqual(
            sr._setting_value(ET.fromstring('<setting id="x"><value>child</value></setting>')),
            "child",
        )
        ...
        self.assertEqual(
            sr.compatibility_preset("UDP-203", jailbreak=True)["oppo_http_payload_mode"],
            "json_payload",
        )
...
class TCoverageGateFourthPassGradual99(unittest.TestCase):
    ...
    def test_discovery_cache_and_probe_edge_paths(self):
        ...
        devs = discovery.discover(
            ssdp=lambda: (_ for _ in ()).throw(RuntimeError("ssdp")),
        ...
        class MemFS:
            ...
            def exists(self, p):
                return self.text is not None or p in self.writes

            def read(self, p):
                return self.text if self.text is not None else self.writes[p]
        ...
        cache.add_many(
            [{"ip": "192.0.2.60", "port": "23", "vendor": "Magnetar", "last_seen": 190.0}, None]
        )
    ...
    def test_external_player_idle_suppression_and_disabled_tv_paths(self):
        ...
        with (
            mock.patch.object(ep, "_safe_tv_switch") as sw,
    ...
    def test_oppo_remote_error_fallbacks_and_hardware_gate_paths(self):
        ...
        calls = []
        with (
            mock.patch.object(remote, "get_audio_tracks", return_value=[]),
        ...
        with (
            mock.patch.object(remote, "_load_settings", return_value=settings),
            mock.patch.object(
                remote, "_command_map", return_value={"bad": "#SRC 6", "missing": ""}
            ),
            mock.patch.object(remote, "send_commands", lambda *a, **k: calls.append(a[2])),
        ):
    ...
    def test_small_manager_logger_and_playercore_edges(self):
        ...
        fs = MemLogFS()
        fs.files["log"] = "old"
        logger = logging_v116.Logger(
            "log", level="DEBUG", max_bytes=1, backups=0, fs=fs, clock=lambda: 0
        )
        logger.info("ip %s", "192.0.2.10")
        self.assertIn("x.x.x.x", fs.files["log"])
        logger.log("TRACE", "hidden")
        with self.assertRaises(ValueError):
            logging_v116.Logger("x", level="NOPE")
        with self.assertRaises(ValueError):
        ...
        with self.assertRaises(ValueError):
            pcf.merge("bad.xml", "<bad>", fs=fs2)
        fs2.files["bad.xml"] = "<broken>"
        with self.assertRaises(ValueError):
            pcf.merge("bad.xml", snippet, fs=fs2)
        ...
        with self.assertRaises(ValueError):
            pm.export_submission("custom", user_preset={"label": "x"})
        with self.assertRaises(ValueError):
            pm.save_submission("bad", tempfile.gettempdir())
        ...
        wfs = WriteFS()
        path = pm.save_submission(
            {"preset_id": "unsafe/id", "preset": {}}, tempfile.gettempdir(), now=lambda: 0, fs=wfs
        )
...
class TCoverageGateFifthPassGradual99(unittest.TestCase):
    def test_external_player_idle_confirmed_and_remote_import_edges(self):
        ...
        with (
            ...
            mock.patch.object(ep.time, "sleep", lambda *_: None),
            mock.patch.object(
                ep, "get_playback_info", side_effect=[{"status": "STOP"}, {"status": "STOP"}]
            ),
            mock.patch.object(ep, "http_info_is_definitive_stop", return_value=False),
        ...
        with (
            mock.patch.object(sr, "warn_if_unsupported", side_effect=RuntimeError("warn failed")),
            mock.patch.object(remote, "_load_settings", return_value=settings),
...
class TCoverageGateBuild3Gradual99(unittest.TestCase):
    def test_small_branch_closures_reconnect_settings_and_tv(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            Path(td, "settings.xml").write_text(
                '<settings><setting id="oppo_start_mode" value="99"/></settings>', encoding="utf-8"
            )
        ...
        with mock.patch.object(tv_control.urllib.request, "urlopen", return_value=bad):
            with self.assertRaises(tv_control.TVControlError):
                tv_control._sony_set_hdmi(
                    FakeSettings({"tv_ip": "192.0.2.3", "sony_psk": "psk"}), 2
                )
        with mock.patch.object(
            tv_control.urllib.request,
            "urlopen",
            side_effect=tv_control.urllib.error.URLError("down"),
        ):
            with self.assertRaises(tv_control.TVControlError):
                tv_control._sony_set_hdmi(
                    FakeSettings({"tv_ip": "192.0.2.3", "sony_psk": "psk"}), 2
    ...
    def test_oppo_tcp_client_direct_socket_edges(self):
        ...
        delays = []
        with (
            mock.patch.object(otc.time, "time", side_effect=[0, 1, 2, 999]),
            mock.patch.object(otc.time, "sleep", lambda s: delays.append(s)),
        ):
            self.assertFalse(
                client.wait_for_stop_persistent(
                    timeout=None,
                    max_retries=2,
                    base_delay=1,
                    cap_delay=1,
    ...
    def test_oppo_control_error_and_filelist_edges(self):
        ...
        with (
            mock.patch.object(oc.socket, "socket", side_effect=sockets),
        ...
        self.assertEqual(
            oc._normalise_filelist_entry({"fileName": "clip.m2ts", "size": "abc", "isDir": "file"})[
                "size_bytes"
            ],
            None,
        )
    ...
    def test_logger_playercore_and_preset_more_edges(self):
        ...
        fs = FS()
        logger = logging_v116.Logger(
            "log", level="DEBUG", max_bytes=100, backups=2, fs=fs, clock=lambda: 1
        )
        ...
        merged, added = pcf._merge_xml(
            ...
            pcf.generate("oppo203", player_path="/tmp/player.py"),
        )
        ...
        submission = pm.export_submission(
            "custom", ip="192.0.2.70", quirks=["clone"], contact="tester", user_preset=valid
        )
...
class TCoverageGateBuild4Gradual99(unittest.TestCase):
    def test_oppo_control_remaining_http_and_parser_edges(self):
        ...
        with mock.patch.object(
            ...
            return_value=json.dumps(
                {"audio_list": {"0": {"index": "2", "name": "Main", "selected": ""}}}
            ),
        ):
        ...
        with mock.patch.object(
            ...
            "_http_get",
            return_value=json.dumps(
                {"subtitle_list": {"0": {"index": "3", "name": "Sub", "selected": True}}}
            ),
        ):
        ...
        with mock.patch.object(
            oc,
            "_http_get",
            side_effect=lambda s, ep, query=None: captured.append((ep, query)) or "ok",
        ):
        ...
        self.assertEqual(
            oc.parse_undocumented_file_list('[{"name":"A.iso"}]', base_path="/base")[0]["path"],
            "/base/A.iso",
        )
        self.assertEqual(
            oc.parse_undocumented_file_list('{"files":[{"name":"B.iso"}]}', base_path="/base")[0][
                "path"
            ],
            "/base/B.iso",
        )
        self.assertEqual(
            oc.parse_undocumented_file_list('{"result":{}}')[0]["entry_type"], "unknown"
        )
        self.assertEqual(
            oc._normalise_filelist_entry(["folder/"], base_path="/base")["entry_type"], "directory"
        )
        self.assertEqual(
            oc._normalise_filelist_entry(["size=abc", "44", "clip.m2ts"], base_path="/base")[
                "size_bytes"
            ],
            44,
        )
    ...
    def test_external_player_timeout_and_preflight_failure_paths(self):
        ...
        with (
            ...
            mock.patch.object(ep, "mark_session_active", lambda s: calls.append("mark")),
            mock.patch.object(
                ep, "fast_start", lambda *a, **k: calls.append(("fast", k.get("preflight_result")))
            ),
            mock.patch.object(ep, "hold_playback", lambda s: calls.append("hold")),
            mock.patch.object(ep, "fast_return", lambda s: calls.append("return")),
            mock.patch.object(ep, "clear_session_active", lambda s: calls.append("clear")),
    ...
    def test_oppo_tcp_wait_timeout_and_close_error_branches(self):
        ...
        with (
            mock.patch.object(otc.time, "time", side_effect=[0, 0.01, 0.02, 1.0]),
        ...
        sleeps = []
        with (
            mock.patch.object(otc.time, "time", side_effect=[0, 0.1, 0.2, 0.3]),
...
class TCoverageGateBuild5Gradual98(unittest.TestCase):
    def test_oppo_control_additional_protocol_and_discovery_edges(self):
        ...
        self.assertEqual(oc._parse_response("@QPW OK"), "")
        self.assertIsNone(
            oc.maybe_setup_verbose_mode(FakeSettings({"oppo_verbose_mode": "0"}), "h", 23)
        )
        self.assertIsNone(
            oc.maybe_setup_verbose_mode(FakeSettings({"oppo_verbose_mode": ""}), "h", 23)
        )
        ...
        self.assertFalse(
            oc.http_info_indicates_playing({"playinfo": "not-a-dict", "state": object()})
        )
        self.assertEqual(
            list(oc._info_containers({"result": {"status": "STOP"}, "playinfo": "bad"}))[1][
                "status"
            ],
            "STOP",
        )
        ...
        calls = []
        with (
            mock.patch.object(oc, "maybe_wake_on_lan", lambda s: calls.append("wol")),
            ...
            self.assertIsNone(
                oc.run_start(
                    FakeSettings(
                        {"oppo_ip": "h", "oppo_port": "23", "oppo_start_mode": "tcp_commands"}
                    ),
                    "/movie.iso",
                )
            )
        ...
        with mock.patch.object(
            ...
            "_http_get",
            return_value=json.dumps(
                {"result": {"0": {"index": 0, "name": "A", "selected": False}}}
            ),
        ):
        ...
        with (
            mock.patch.object(oc.socket, "socket", side_effect=[main_sock, probe_sock]),
    ...
    def test_external_player_remaining_cleanup_and_reset_paths(self):
        ...
        calls = []
        with (
            mock.patch.object(ep, "run_start", lambda *a, **k: calls.append("run_start")),
            mock.patch.object(ep.time, "sleep", lambda *_: calls.append("sleep")),
        ):
            ep.start_oppo_after_optional_delay(
                FakeSettings({"startup_delay": "0", "oppo_start_mode": "tcp_commands"}), "/m.iso"
            )
        self.assertEqual(calls, ["run_start"])
        with (
            mock.patch.object(ep, "_safe_tv_switch", lambda *a, **k: calls.append("tv")),
            mock.patch.object(
                ep, "start_oppo_after_optional_delay", lambda *a, **k: calls.append("start")
            ),
        ):
        ...
        with (
            ...
            mock.patch.object(
                ...
                "read_settings",
                return_value=FakeSettings(
                    {"addon_data_dir": tempfile.gettempdir(), "oppo_preflight_enabled": "false"}
                ),
            ),
            mock.patch.object(ep, "mark_session_active", lambda s: calls.append("mark")),
            mock.patch.object(
    ...
    def test_settings_logger_and_merge_edge_branches(self):
        ...
        class FS:
            def __init__(self):
                self.files = {"log": "old", "log.1": "older"}
                self.removed = []
        ...
        fs = FS()
        logger = logging_v116.Logger(
            "log", level="INFO", max_bytes=100, backups=1, fs=fs, clock=lambda: 0
        )
        logger.rotate()
        self.assertIn("log.1", fs.files)
        ...
        existing = (
            ...
            + snippet.split("<players>", 1)[1].split("</players>", 1)[0]
            + "</players><rules></rules></playercorefactory>"
        )
