...
def test_build11_metadata_and_documentation_identity():
    ...
    assert (ROOT / "BUILD_NOTES_v2.9.10_BUILD11.md").exists() or (
        ROOT / "docs" / "release-history" / "BUILD_NOTES_v2.9.10_BUILD11.md"
    ).exists()
    assert (ROOT / "HARDWARE_ECOSYSTEM_SUPPORT_MATRIX_v2.9.10_BUILD11.md").exists() or (
        ROOT / "docs" / "release-history" / "HARDWARE_ECOSYSTEM_SUPPORT_MATRIX_v2.9.10_BUILD11.md"
    ).exists()
...
def test_build11_invalid_backend_and_custom_message_are_nonfatal():
    ...
    assert "avr_config_incomplete" in validation["warnings"]
    result = avr_control.build_nonfatal_result(
        "restore", settings, message="manual warning"
    ).as_dict()
