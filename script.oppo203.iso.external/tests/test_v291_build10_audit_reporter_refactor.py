...
def test_audit_check_value_object_and_legacy_dict_api_are_both_available():
    ...
    legacy = audit.run_audit(audit.project_root(audit.Path(ROOT)), expected_version="2.9.12")
    typed = audit.collect_audit_checks(audit.project_root(audit.Path(ROOT)), expected_version="2.9.12")
    assert legacy
    assert typed
...
def test_audit_cli_text_and_json_outputs_still_work():
    text = subprocess.run(
        [sys.executable, str(ROOT / "tools" / "audit_release.py"), "--root", str(ROOT), "--expected-version", "2.9.12"],
        check=False,
        text=True,
    ...
    json_result = subprocess.run(
        [sys.executable, str(ROOT / "tools" / "audit_release.py"), "--root", str(ROOT), "--expected-version", "2.9.12", "--json"],
        check=False,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
