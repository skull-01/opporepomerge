...
def test_created_runtime_zip_uses_allowlist_and_has_no_dev_artifacts(tmp_path):
    tool = _load_package_tool()
    out = tmp_path / "script.oppo203.iso.external-2.9.10.zip"
...
def test_package_release_script_defaults_to_active_build(tmp_path):
    ...
    runtime_zip = out / "script.oppo203.iso.external-2.9.10.zip"
    dev_zip = out / "script.oppo203.iso.external-2.9.10-dev-source.zip"
    checksum = out / "script.oppo203.iso.external-2.9.10.sha256"
