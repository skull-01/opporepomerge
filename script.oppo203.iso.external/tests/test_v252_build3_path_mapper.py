...
def test_build3_metadata_and_release_audit_evidence():
    ...
    spec = importlib.util.spec_from_file_location(
        "audit_release_v252_build3", ROOT / "tools" / "audit_release.py"
    )
...
def test_path_mapper_defensive_and_normalization_branches():
    ...
    dict_rule = {
        "kodi_prefix": "SMB://TRUENAS/media",
        "player_prefix": "/mnt/nas/media",
        "label": "dict",
    }
    ...
    assert (
        normalize_kodi_path("smb://truenas/media/Movie.iso?x=1#frag")
        == "smb://truenas/media/Movie.iso?x=1#frag"
    )
