...
def test_build16_http_401_403_and_network_errors_are_nonfatal():
    ...
    auth_result = avr_sony_audio.send_sony_audio_request(
        _sony_settings(), "getPowerStatus", [], post=fake_401
    ).as_dict()
    ...
    timeout_result = avr_sony_audio.send_sony_audio_request(
        _sony_settings(), "getPowerStatus", [], post=fake_timeout
    ).as_dict()
...
def test_build16_invalid_json_api_error_and_secret_echo_are_sanitized():
    ...
    result = avr_sony_audio.send_sony_audio_request(
        settings, "getPowerStatus", [], post=invalid_json
    ).as_dict()
    assert result["ok"] is False
    ...
    error_result = avr_sony_audio.send_sony_audio_request(
        settings, "getPowerStatus", [], post=api_error
    ).as_dict()
...
def test_build16_url_payload_validation_and_incomplete_config_are_nonfatal():
    ...
    assert (
        avr_sony_audio.build_sony_audio_url("http://sony.local", "/sony/audio")
        == "http://sony.local/sony/audio"
    )
    assert (
        json.loads(avr_sony_audio.build_power_on_payload().decode("utf-8"))["method"]
        == "setPowerStatus"
    )
    assert (
        json.loads(avr_sony_audio.build_set_input_payload("extInput:hdmi?port=1").decode("utf-8"))[
            "method"
        ]
        == "setPlayContent"
    )
    assert (
        json.loads(avr_sony_audio.build_status_payload().decode("utf-8"))["method"]
        == "getPowerStatus"
    )
...
def test_build16_sony_edge_paths_raise_no_secrets_and_remain_nonfatal():
    ...
    assert (
        avr_sony_audio.is_experimental_acknowledged({"sony_avr_experimental_acknowledged": ""})
        is False
    )
    assert (
        avr_sony_audio.send_sony_audio_request(
            _sony_settings(avr_timeout="bad"), "getPowerStatus", [], post=lambda *a: "{}"
        ).as_dict()["ok"]
        is True
    )
    assert (
        avr_sony_audio.send_sony_audio_request(
            _sony_settings(avr_timeout="99"), "getPowerStatus", [], post=lambda *a: "{}"
        ).as_dict()["ok"]
        is True
    )
    ...
    for host, path in (
        ("", "/sony/audio"),
        ("sony.local/path", "/sony/audio"),
        ("sony.local", "../bad"),
    ):
        result = avr_sony_audio.send_sony_audio_request(
            _sony_settings(avr_host=host, sony_avr_api_path=path),
            "getPowerStatus",
            [],
            post=lambda *a: b'{"result":[]}',
        ).as_dict()
        assert result["ok"] is False
        assert result["nonfatal"] is True
        assert result["command_sent"] is False
    ...
    assert (
        avr_sony_audio.send_sony_audio_request(
            _sony_settings(), "bad/method", [], post=lambda *a: b"{}"
        ).as_dict()["ok"]
        is False
    )
    try:
    ...
    result = avr_sony_audio.send_sony_audio_request(
        _sony_settings(), "getPowerStatus", [], post=boom
    ).as_dict()
    assert result["ok"] is False
    assert result["warnings"] == ("avr_driver_error_nonfatal",)
    assert "supersecretpsk" not in str(result)

    no_input = avr_sony_audio.SonyAudioApiAvrController(
        _sony_settings(sony_avr_player_input_uri="", avr_player_input="")
    )
...
def test_build16_onkyo_eiscp_edge_paths_stay_nonfatal():
    ...
    assert avr_onkyo_eiscp.parse_eiscp_response(
        avr_onkyo_eiscp.build_eiscp_frame("!1PWR01").decode("ascii", errors="ignore")
    ).startswith("!1PWR01")

    for frame in (
        b"",
        b"BADC" + b"\x00" * 12,
        b"ISCP" + b"\x00\x00\x00\x10" + b"\x00\x00\x00\xff" + b"\x01\x00\x00\x00",
    ):
        try:
    ...
    assert avr_onkyo_eiscp.send_eiscp_command("", "!1PWR01").as_dict()["warnings"] == (
        "avr_config_incomplete",
    )
    assert avr_onkyo_eiscp.send_eiscp_command(
        "avr.local", "bad\n", socket_factory=lambda *a: object()
    ).as_dict()["warnings"] == ("invalid_eiscp_payload",)
    ...
    ok = avr_onkyo_eiscp.send_eiscp_command(
        "avr.local", "!1PWR01", socket_factory=lambda *a: ClosingSocket()
    ).as_dict()
    assert ok["ok"] is True

    controller = avr_onkyo_eiscp.OnkyoEiscpAvrController(
        "avr.local", player_input="", socket_factory=lambda *a: ClosingSocket()
    )
