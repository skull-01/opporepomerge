...
def test_addon_xml_matches_version_source():
    ...
    assert 'version="2.9.11"' in addon_text
    assert "Version 2.9.1 Build 11" in addon_text


def test_sync_version_check_reports_consistency():
    tool = _load_sync_version()
    ok, detail = tool.check_version_consistency(ROOT, expected_version="2.9.11")
    assert ok is True
    assert "addon.xml and version.py agree on 2.9.11" in detail
    assert tool.version_source(ROOT) == "2.9.11"
    assert tool.addon_xml_version(ROOT) == "2.9.11"


def test_sync_version_cli_check_passes():
    result = subprocess.run(
        [sys.executable, str(ROOT / "tools" / "sync_version.py"), "--root", str(ROOT), "--check", "--expected-version", "2.9.11"],
        check=False,
        text=True,
...
def test_sync_version_write_only_changes_addon_attribute_in_copy(tmp_path):
    ...
    assert written == "2.9.11"
    assert ET.parse(sandbox / "addon.xml").getroot().attrib["version"] == "2.9.11"
