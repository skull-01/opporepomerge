...
def test_sync_version_check_reports_consistency():
    ...
    ok, detail = tool.check_version_consistency(ROOT, expected_version="2.9.16")
    assert ok is True
    assert "addon.xml and version.py agree on 2.9.16" in detail
    assert tool.version_source(ROOT) == "2.9.16"
    assert tool.addon_xml_version(ROOT) == "2.9.16"
...
def test_sync_version_write_only_changes_addon_attribute_in_copy(tmp_path):
    ...
    assert written == "2.9.16"
    assert ET.parse(sandbox / "addon.xml").getroot().attrib["version"] == "2.9.16"
    assert (
        (sandbox / "addon.xml")
