...
def test_g7_did_not_change_runtime_packaging_allowlist() -> None:
    ...
    assert package_tool.is_runtime_member("tests/test_github_readiness_g7_safe_format_cleanup.py") is False
...
def test_g7_archive_records_safe_cleanup_scope() -> None:
    archived = (ROOT / "docs" / "github-readiness" / "AI_HANDOFF_G7_SAFE_FORMAT_LINT.md").read_text(encoding="utf-8")
    assert "G7" in archived
    assert "Safe Format" in archived or "safe format" in archived
    assert "runtime_behavior_changed: false" in archived or "Runtime behavior changed: false" in archived
