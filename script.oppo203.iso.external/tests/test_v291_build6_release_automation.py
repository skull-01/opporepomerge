...
def test_package_release_script_creates_runtime_zip_dev_source_and_checksum(tmp_path):
    ...
    result = subprocess.run(
        ...
        check=False,
        cwd=str(ROOT),
        env={**os.environ, "OUT_DIR": str(out), "VERSION": "2.9.17", "BUILD_SUFFIX": "build6-test"},
        text=True,
        capture_output=True,
    )
    assert result.returncode == 0, result.stdout + result.stderr
    runtime_zip = out / "script.oppo203.iso.external-2.9.17-build6-test.zip"
    dev_zip = out / "script.oppo203.iso.external-2.9.17-build6-test-dev-source.zip"
    checksum = out / "script.oppo203.iso.external-2.9.17-build6-test.sha256"
