...
/**
 * The canonical playback-architecture preset matrix - the cross-product of 3 routing modes x 2
 * monitor modes (legacy, svm3) = 6 base presets, PLUS the asymmetric 7th cell http_handoff_http
 * (the http monitor exists only for the http_handoff routing, not a full column). This JSON is the
 * single cross-language source of truth, shared with the add-on (resources/lib/kodi/settings_reader.py)
 * and pinned to its Python registries by tests/test_playback_presets_consistency.py. Changing the
 * set is a deliberate cross-area contract change - update both sides + this file in lock-step
 * (AGENTS.md: "six playback-architecture presets are a maintained matrix").
 */
...
/** The seven canonical preset ids, in the add-on's PLAYBACK_ARCHITECTURE_PRESETS order. */
