...
class App:
    def __init__(self, root: "tk.Tk"):
        ...
        ttk.Label(frm, text="OPPO model").grid(row=1, column=0, sticky="w", **pad)
        self.oppo_model = tk.StringVar(value=self.cfg.get("oppo_model", "M9205"))
        ttk.Combobox(frm, textvariable=self.oppo_model, width=21, state="readonly",
                     values=OPPO_MODELS).grid(row=1, column=1, sticky="ew", **pad)

        ttk.Label(frm, text="Kodi IP").grid(row=2, column=0, sticky="w", **pad)
        self.kodi_ip = tk.StringVar(value=self.cfg["kodi_ip"])
        ttk.Entry(frm, textvariable=self.kodi_ip, width=24).grid(row=2, column=1, sticky="ew", **pad)

        ttk.Label(frm, text="Kodi port").grid(row=3, column=0, sticky="w", **pad)
        self.kodi_port = tk.StringVar(value=str(self.cfg["kodi_port"]))
        ttk.Entry(frm, textvariable=self.kodi_port, width=8).grid(row=3, column=1, sticky="w", **pad)

        row = ttk.Frame(frm)
        row.grid(row=4, column=0, columnspan=2, pady=(2, 6))
        ...
        ttk.Separator(frm, orient="horizontal").grid(row=5, column=0, columnspan=2, sticky="ew", pady=6)

        self.btn_oppo = ttk.Button(frm, text="▶  Switch to OPPO", command=self.on_oppo)
        self.btn_oppo.grid(row=6, column=0, columnspan=2, sticky="ew", pady=4, ipady=10)
        self.btn_kodi = ttk.Button(frm, text="\U0001f3e0  Switch to Kodi", command=self.on_kodi)
        self.btn_kodi.grid(row=7, column=0, columnspan=2, sticky="ew", pady=4, ipady=10)

        self.log = tk.Text(frm, height=8, width=46, state="disabled", wrap="word")
        self.log.grid(row=8, column=0, columnspan=2, sticky="nsew", pady=(8, 0))
    ...
    def _current(self) -> dict:
        ...
        return {
            ...
            "oppo_model": self.oppo_model.get(),
    ...
    def on_oppo(self) -> None:
        cfg = self._current()
        self._log("Switch to OPPO ({}): {} (TV follows in ~20-24s) ...".format(cfg["oppo_model"], cfg["oppo_ip"]))
        self._async(lambda: cec_core.oppo_take_tv(cfg["oppo_ip"], model=cfg["oppo_model"]))
